...
class GCDViewController: UIViewController {
    ...
    //并发队列: 任务以FIFO从序列中移除,然后并发运行,可以按照任何顺序完成。用多条线程执行队列中任务
    ...
    //串行队列: 任务以FIFO从序列中一个一个执行。一次只调度一个任务,队列中的任务一个接着一个地执行（一个任务执行完毕后,再执行下一个任务）用一条线程处理队列中任务
    ...
    @IBAction func action03(_ sender: UIButton) {
        ...
        for i in 1...20 {
            queue.async {
                semaphore.wait() // 如果信号量计数>=1,将信号量计数减1;当信号量等于0时等待
                let time:UInt32 = arc4random() % 10
                Thread.sleep(forTimeInterval: TimeInterval(time))
                print("finished:\(i) --> \(Thread.current)")
    ...
    @IBAction func manyTagetAction(_ sender: UIButton) {
        ...
        group.notify(queue: queue) {
            print("group done --> \(Thread.current)")
        }
        print("line \(#line)")
    ...
    @IBAction func targetFinished(_ sender: UIButton) {
        ...
        queue.async(group: group) {
            ...
            myqueue.asyncAfter(deadline: .now() + 2.0) {//在子线程执行
                print("异步网络任务1:请求完成! --> \(Thread.current)")
                group.leave()
            }
            print("finished 1")
        }
        ...
        queue.async(group: group) {
            DispatchQueue.global().async {
                print("异步网络任务2:请求完成! --> \(Thread.current)")
                group.leave()
            }
            print("finished 2")
        }
        ...
        queue.async(group: group) {
            DispatchQueue.global().asyncAfter(deadline: .now() + 3.0, execute: {
                print("异步网络任务3:请求完成! --> \(Thread.current)")
                group.leave()
            })
            print("finished 3")
        }
        ...
        /**
         可以试下去掉所有的group.enter()和group.leave(),去掉后异步任务还没执行完,group.notify()就开始执行了
        ...
        group.notify(queue: DispatchQueue.main) {
            print("所有网络任务都完成了,刷新页面 --> \(Thread.current)")
        }
        print("接下来的操作   1")
    ...
    @IBAction func barrierAction(_ sender: UIButton) {
        ...
        currentQueue.async {
            sleep(2)
            print("target1 finished --> \(Thread.current)")
        }
        
        currentQueue.async {
            sleep(1)
            print("target2 finished --> \(Thread.current)")
        }
        //障碍,等队列前面的所有任务都执行完,后面的才会执行
        currentQueue.async(flags: .barrier) {
            print("taget1 target2 is finished --> \(Thread.current)")
        }
        
        currentQueue.async {
            sleep(1)
            print("target3 finished --> \(Thread.current)")
        }
