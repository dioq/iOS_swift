...
class NativeNetworkVC: UIViewController {
    ...
    @IBAction func get_func(_ sender: UIButton) {
        ...
        NetworkManager.shareManager().doGet(urlStr: urlStr) { (response) in
            print(response)
        } failure: { (error) in
            print(error)
        }
    ...
    @IBAction func formdata_func(_ sender: UIButton) {
        ...
        let param = NetworkManager.shareManager().dictToFormdataStr(dict: param_dict)
        NetworkManager.shareManager().submitFormdata(urlStr: urlStr, params: param) { (response) in
            print(response)
        } failure: { (error) in
            print(error)
        }
    ...
    @IBAction func uploadImage_func(_ sender: UIButton) {
        ...
        NetworkManager.shareManager().uploadImage(urlStr: urlStr, image: image, withName: "file", fileName: "0001") { (response) in
            print(response)
        } failure: { (error) in
            print(error)
        }
    ...
    @IBAction func uploadBinary_func(_ sender: UIButton) {
        ...
        let image:UIImage = UIImage.init(named: "pingtaoge")!
        NetworkManager.shareManager().uploadBinary(urlStr: urlStr, binaryFile: image) { (response) in
            print(response)
        } failure: { (error) in
            print(error)
        }
